...
import { updateDatabase, cuentasStreamingUrl } from '../firebase'

import { addMonth, format } from "@formkit/tempo"
import { toast, Toaster } from "sonner";

function ActualizarCuenta({cuentaActualizar = {}}) {
  const { 
    correo = '',
    password = '',
    planSuscripcion = '',
  ...
  const [ correoIn, setCorreoIn ] = useState(correo)
  const [ passwordIn, setPasswordIn ] = useState(password)
  const [ planSuscripcionIn, setPlanSuscripcionIn ] = useState(planSuscripcion)
  const [ tiempoServicioIn, setTiempoServicioIn ] = useState(tiempoServicio)
  ...
  async function newAccount(e) {
    ...
    const fechaFinal = addMonth(fechaInicioIn, Number(tiempoServicioIn))
    const plan = planesSuscripcion.find(plan => plan.tipo == planSuscripcionIn)

    const datos = {
      correo: correoIn,
      password: passwordIn,
      planSuscripcion: planSuscripcionIn,
      espacios: plan.espacios,
      tiempoServicio: Number(tiempoServicioIn),
      fechaInicio: fechaInicioIn,
      fechaFinal: format(fechaFinal, 'YYYY-MM-DD', 'es'),
      fechaAgregado
    }

    await updateDatabase(cuentasStreamingUrl, id, datos)
  ...
  return (
    <div>
      ...
      >
        <h2 className="text-lg md:text-2xl">Actualiza Cuenta Streaming</h2>
        <form  
          className="mt-4"
        ...
        >
          <CampoCorreo 
            value={correoIn}
            setValue={setCorreoIn}
          />
          <CampoPassword 
            value={passwordIn}
            setValue={setPasswordIn}
          />
          <SelectOption 
            title='Plan Suscripción'
            value={planSuscripcionIn}
            setValue={setPlanSuscripcionIn}
            options={planesSuscripcion.map(plan => plan.tipo)}
            forName='planes'
          />
          <SelectOption 
            title='Duración de Servicio'
            value={tiempoServicioIn}
            setValue={setTiempoServicioIn}
